...
export default async function handler(request, response) {
  ...
  const RESPONSE_AUDIO_CREATED = { status: "audio created" };
  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      const existingAudio = await Audio.findOne({ serialnumber });
      if (name === "") {
      ...
      if (existingAudio) {
        return response.status(HTTP_STATUS_BAD_REQUEST).json({
          error: `"${serialnumber}" is in the database. Change "${existingAudio}" in the form`,
        });
      } else {
      ...
      response
        ...
        .json(RESPONSE_AUDIO_CREATED, audio, { success: "Form is valid" });
    } else if (!allowedMethods.includes(request.method)) {
      return response
        .status(HTTP_STATUS_METHOD_NOT_ALLOWED)
        .json({ message: "Method not allowed for audio details" });
    }
  } catch (error) {
